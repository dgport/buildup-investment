...
const ForgotPasswordForm = () => {
  ...
  const handleSubmit = () => {
    ...
    forgotPasswordMutation.mutate({ email }, { onSuccess: () => setEmail("") });
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !forgotPasswordMutation.isPending) handleSubmit();
  };
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              autoComplete="email"
              className={`bg-teal-950/50 border-amber-400/20 text-white placeholder:text-amber-100/30 focus:border-amber-400 focus:ring-amber-400/20 ${fieldError ? "border-red-500" : ""}`}
            />
