...
export default function SigninPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "email" || name === "password") setGoogleAccountEmail(null);
  };
  ...
  const validateForm = () => {
    ...
    else if (formData.password.length < 6) errors.password = t("passwordMin");
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !signInMutation.isPending) handleSubmit();
  };
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                autoComplete="email"
                className={`bg-teal-950/50 border-amber-400/20 text-white placeholder:text-amber-100/30 focus:border-amber-400 focus:ring-amber-400/20 ${fieldErrors.email ? "border-red-500" : ""}`}
              />
              {fieldErrors.email && (
